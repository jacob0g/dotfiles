---
name: converting-to-tailwind
description: Convert plain CSS stylesheets or CSS Modules (.module.css/.module.scss) to Tailwind CSS utility classes. Handles selectors, media queries, pseudo-classes, custom properties, animations, styles object removal, className interpolation, composition, and global overrides.
user-invocable: true
---

# Converting CSS to Tailwind

Migrate plain CSS files or CSS Modules to Tailwind utility classes applied directly in markup.

## Workflow

1. **Read the CSS file** and inventory every rule
2. **Find the corresponding markup** (HTML, JSX, TSX, Vue, Svelte) that references each selector (or `styles.xxx` for CSS Modules)
3. **Convert each rule** using the reference tables and patterns below
...
When migrating `.module.css` / `.module.scss` files, handle these additional patterns.

### Inventorying a Module
...
- **Pseudo-elements** (`:before`, `:after` with `content`) — use `before:` / `after:` prefixes plus `content-['...']` in Tailwind
- **`:nth-child`, `:first-of-type`**, etc. — check if Tailwind has a matching variant, otherwise keep a minimal CSS rule
